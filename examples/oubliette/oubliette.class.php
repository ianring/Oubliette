...
class Oubliette {
	...
	function log($log){
		...
		file_put_contents(OUBLIETTE_LOG, json_encode($log)."\n", FILE_APPEND);
		
		$log = array_merge($log, array(
			'reporter_host'=>$_SERVER['HTTP_HOST'],
			'reporter_ip' => $_SERVER['SERVER_ADDR'],
			'timestamp'=>time()
		));
		
		if ($this->config['enable_hive'] == 'true'){
			$this->hive_send($log);
		}
	...
	public function test(){
		...
		// if this is the oubliette admin script, then allow it.
		$allowedfolder = OUBLIETTE_ROOT.'/admin';
		if (substr($_SERVER['REQUEST_URI'], 0, strlen($allowedfolder)) == $allowedfolder) {
			return true;
		}
		
		if (isset($_POST['oubliette_action']) && isset($_POST['challenge'])){
			$win = $this->accept_challenge();
			if ($win){
				$unbanned = $this->forgive();
				if ($unbanned) {
					
					$uri = $_SERVER['REQUEST_URI'];
					$uri = str_replace("?oubliette_unban", "", $uri);
					$uri = str_replace("&oubliette_unban", "", $uri);
					if (strpos($uri,'oubliette') !== false){
						$uri = OUBLIETTE_ROOT."/../";
					}
					header("Location: ".$uri);
				} else {
					$this->render_page('unban_fail');
				}
			} else {
				$this->render_page('unban_fail');
			}
		}
		if (isset($_GET['oubliette_unban'])){
			$this->render_page('unban_challenge');
		}
		...
		if ($this->is_greylisted()){
			$this->render_page('greylisted',array(
				'duration' => $this->config['grey_time_limit']
			));
			die();
		}
		if ($this->ratelimit()){
...
//		echo $this->config['rate_visits'];
		if ($r >= $this->config['rate_visits']){
			$this->log(array(
				'ip'=>$ip,
				'rate_seconds'=>$this->config['rate_seconds'],
				'rate_visits'=>$this->config['rate_visits'],
	...
	function honeypot(){
		...
		$html .= '<a href="'.OUBLIETTE_ROOT.'/index.php" class="honeypot1">click here to download all the internets</a>';
		return $html;
	}
	...
	*/
	...
	function challenge(){
		...
		$hash2 = sha1($sum . $hash . $nonce . OUBLIETTE_SALT);
		
		$html = '';
		$html .= '<form action="" method="post">';
		$html .= '<input type="hidden" name="oubliette_action" value="unban" />';
		$html .= '<input type="hidden" name="challenge" value="'.$a.'|'.$nonce.'|'.$b.'|'.$hash.'" />';
		$html .= '<label>Type the answer to this question: <b>'.$a.' + '.$b.' = ______ ?</b> </label>';
		$html .= '<input type="text" name="'.$hash2.'" value="" />';
	...
	function accept_challenge(){
		list($a, $nonce, $b, $hashin) = explode("|",$_POST['challenge']);
		$sum = $a + $b;
		$hash = md5(OUBLIETTE_SALT . $sum);
		$hash2 = sha1($sum . $hash . $nonce . OUBLIETTE_SALT);
	...
	function render_page($page,$params=array()){
//		print_r($params);
		switch($page){
			case 'blacklisted':
				header("HTTP/1.0 403 Forbidden");
				$params = array_merge($params,array(
					'icon'=>OUBLIETTE_ROOT.'/assets/icon01.png'
				));
				$p = $this->render_template("blacklisted",$params);
				echo $p;
				die();
				break;
			case 'blacklisted_with_unban':
				header("HTTP/1.0 403 Forbidden");
				$unbanlink = $_SERVER['REQUEST_URI'];
				if (strpos($unbanlink, "?") !== FALSE){
					$unbanlink .= "&oubliette_unban";
				} else {
					$unbanlink .= "?oubliette_unban";
				}
				...
				$p = $this->render_template("blacklisted_with_unban",$params);
				echo $p;
				die();
				break;
			case 'ratelimited':
				...
				$p = $this->render_template("ratelimited",$params);
				echo $p;
				die();
				break;
			case 'greylisted':
				...
				$p = $this->render_template("greylisted",$params);
				echo $p;
				die();
				break;
			case 'unban_challenge':
				// returnto was passed in...
				$params = array_merge($params,array(
					'challenge'=>$this->challenge($params['returnto'])
				));
				$p = $this->render_template("unban_challenge",$params);
				echo $p;
				die();
				break;
			default:
	...
	private function render_template($template,$params = array()){
		$t = file_get_contents(OUBLIETTE_PATH."/templates/".$template.".tpl");
	...
	private function hive_send($data){
		
		$json = json_encode($data);
		
		$ch = curl_init();
		curl_setopt($ch,CURLOPT_URL, "http://oubliette.grawlix.com/hive/");
